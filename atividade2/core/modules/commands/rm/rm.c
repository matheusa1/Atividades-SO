...
int rm_arq_entry(FAT32_DirEntry* entry, FILE* file, uint32_t* fat, FAT32_BootSector boot_sector, FSInfoStruct* fsi_info){
...
void rm_command(char *entry_name, Fat32Image *image, uint32_t current_cluster) {
    ...
    do {
        ...
        // first_data_sector = setores reservados + (número de FATs * setores por FAT)
